...
class TemplateServer(webapp.RequestHandler):
  ...
  def get(self):
    ...
    if extension in ('djhtml', 'django', 'mango', 'html', 'htm'):
...
def GetPath():
  ...
  if os.environ['PATH_INFO'] == '/':
    paths = [os.path.join(os.path.dirname(__file__), 'index.%s' % ext) 
               for ext in ('djhtml', 'mango', 'django', 'html', 'htm')]
  for path in paths:
...
def GetFileNameFromUri(self):
  """Splits a path into several components.
  
  Returns:
    (filename). For example, for 'http://host/index.html', this
...
def Main():
  ...
  if extension in ('djhtml', 'django', 'mango','htm','html', 'htm'):
